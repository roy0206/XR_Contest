...
## 추가 작업 · FixedUI HUD와 시작 화면 적용

R의 후속 승인 아래 `FixedUI.zip`에서 실제 프로젝트에 필요한 UI 자산만 선별해 현재 HUD 계층에 연결했다.

### 일시정지 및 퀘스트 HUD

- 퀘스트 HUD를 실제 `QuestManager` 상태와 연결해 진행·대기·완료·오류를 표시한다.
- FixedUI의 Giants 폰트와 상태별 글래스 패널 스타일을 적용했다.
- 일시정지 메뉴는 원본 아이콘 4종을 사용하며 계속하기·옵션·공정 재시작·메인 화면 기능을 기존 컨트롤러에 연결했다.
- 타 개발자의 공정 판정 코드는 수정하지 않았다.

### 시작 화면

- 원본 ZIP 전체를 임포트하지 않고 `news.fbx`가 사용하는 7개 재질과 해당 재질의 실제 텍스처 참조 18개만 `Assets/@Developers/RYU/Start/FixedUI`로 이관했다.
- `StartMenu.uxml`의 기존 버튼·슬라이더·확인 창 이름은 유지하고 FixedUI 글래스 메뉴 구조로 교체했다.
- `StartMenuController`, `GameFlow`, 저장 초기화 및 씬 전환 코드는 변경하지 않았다.
- `StartMenuBackdrop`은 현재 활성 메인 카메라를 찾은 뒤 모델 높이와 중심을 기준으로 화면 오른쪽에 자동 프레이밍한다. XR 카메라가 늦게 활성화되는 경우에도 생성될 때까지 대기한다.
- 모델에 들어 있는 Collider는 시작 화면 입력을 방해하지 않도록 런타임 인스턴스에서 비활성화한다.
- `StartScene`에는 위 프레젠테이션 어댑터와 원본 모델·재질 참조만 추가했다.

### 추가 검증

- `StartScene_UsesFixedUiPresentationWithoutBreakingMenuContract`를 추가했다.
- 시작 메뉴 필수 요소 15개, FixedUI 원본 아이콘 4개, 모델과 메타, StartScene의 프레젠테이션 컴포넌트·모델 GUID 배선을 검사한다.
- `Assembly-CSharp.csproj`: 오류 0
- `IUM.CoreLoopVerification.Tests.csproj`: 오류 0
- Unity 임포트: 모델·재질·텍스처·UXML·USS 오류 0
- 현재 열린 Unity Editor가 Play Mode 상태에서 외부 요청 파일을 재수신하지 않아, 추가된 계약 테스트의 Editor 실행은 다음 Edit Mode 진입 후 `Tools > IUM > Verify Full Core Loop`에서 재확인한다. 이전 전체 검증 결과는 8/8 통과 상태다.

### 보존한 기존 변경

- `Assets/warehouseFin/Scenes/SampleScene.unity.meta`는 작업 시작 전부터 존재한 미커밋 변경이며 이번 작업에서 수정·스테이징하지 않았다.
